...
@php
	...
	$viewName = 'daterangepicker-datetime';
	$type = 'text';
	$label ??= null;
	$id ??= null;
	$name ??= null;
	...
	$referenceDate ??= null;
	$referenceDate = !empty($referenceDate) ? (DateValidator::isValid($referenceDate) ? $referenceDate : date('Y-m-d')) : null;
	$pluginOptions ??= [];
	...
	$attributes = \App\Helpers\Common\Html\HtmlAttr::append($attributes, 'class', 'be-select-datetime');
...
@pushonce("{$viewName}_helper_scripts")
	<script>
		onDocumentReady((event) => {
			...
			const dateTimeEl = $('.be-select-datetime');
			
			const minYear = {{ $minYear }};
			const maxYear = {{ $maxYear }};
			const datepickerFormat = '{{ t('datepicker_format_datetime') }}';
			...
			dateTimeEl.daterangepicker(options);
			
			dateTimeEl.on('apply.daterangepicker', function (e, picker) {
				if (referenceDate) {
